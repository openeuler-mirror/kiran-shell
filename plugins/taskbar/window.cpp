...
void Window::initWindowManager()
{
    ...
    connect((Applet *)parent(), &Applet::windowChanged, [this](WId wid, NET::Properties properties, NET::Properties2 properties2)
            {
                // 窗口打开后，运行过程中标志变更
                bool hasContains = false;
                for (auto &appGroups : m_mapAppGroupOpened)
                {
                    if (appGroups.second.contains(wid))
                    {
                        hasContains = true;
                        break;
                    }
