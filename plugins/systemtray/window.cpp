...
Window::Window(IAppletImport *import, QWidget *parent)
    : KiranColorBlock(parent), m_import(import), m_windowPopup(nullptr),
...
void Window::startUpdateTrayBoxPos() {
  if (!m_updateWindowPopupPosInProgress) {
    ...
    QTimer::singleShot(100, this, [this]() { updateTrayBoxPos(); });
  }
}
...
void Window::dropEnd(QString serviceAndPath) { itemRemove(serviceAndPath); }
...
QList<TrayItem *> Window::getTrayItems() { return m_services.values(); }

WindowPopup *Window::getTrayBox() { return m_windowPopup; }
