...
bool ThemeConfItem::getTheme(QString &themeID)
{
    ...
    if (message.isError())
    {
        KLOG_WARNING(LCSettingbar) << "getTheme failed:" << message.error();
        return false;
    }

    themeID = message.value();
    KLOG_INFO(LCSettingbar) << "current theme" << themeID;
...
bool ThemeConfItem::setTheme(const QString &themeID)
{
    ...
    KLOG_INFO(LCSettingbar) << "set theme" << themeID;

    auto message = m_ksAppearance->SetTheme(APPEARANCE_THEME_TYPE_GTK, themeID);
    if (message.isError())
    {
        KLOG_WARNING(LCSettingbar) << "setTheme,type:" << APPEARANCE_THEME_TYPE_GTK << "failed:" << message.error();
        return false;
    }
    message = m_ksAppearance->SetTheme(APPEARANCE_THEME_TYPE_METACITY, themeID);
    if (message.isError())
    {
        KLOG_WARNING(LCSettingbar) << "setTheme,type:" << APPEARANCE_THEME_TYPE_METACITY << "failed:" << message.error();
        return false;
    }
