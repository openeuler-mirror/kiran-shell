...
bool Brightness::getBrightness(int &value)
{
    ...
    if (message.isError())
    {
        KLOG_WARNING(LCSettingbar) << "GetBrightness failed:" << message;
        return false;
    }
...
void Brightness::updateBrightness()
{
    ...
    KLOG_INFO(LCSettingbar) << "updateBrightness" << brightnessValue;
}

void Brightness::dbusBrightnessChanged(QDBusMessage message)
{
    KLOG_INFO(LCSettingbar) << "dbusBrightnessChanged" << message;
    updateBrightness();
}

void Brightness::dbusActiveProfileChanged(QDBusMessage message)
{
    KLOG_INFO(LCSettingbar) << "dbusActiveProfileChanged" << message;
    updateBrightness();
}
...
void Brightness::handleSetBrightnessResult(QDBusPendingCallWatcher *watcher)
{
    ...
    if (reply.isError())
    {
        KLOG_WARNING(LCSettingbar) << "SetBrightness failed:" << reply.error().message();
    }
